...
virk_3.5 <- bf(PR ~ 0 + Social_Rep + BotGender + (1| ID))
...
virk_4.5 <- bf(PR ~ 0 + Social_Rep:BotGender + Gender + (0 + Social_Rep:BotGender + Gender | ID))
...
virk_f14 <- bf(PR ~ 0 + Gender + "best from above" + (0 + Gender + "best from above" | ID))
```
...
##### Model 3.5
```{r}
get_prior(virk_3.5, d, family = bernoulli())

prior_f3.5 <- c(
  prior(normal(0,1.5), class = b, coef = "Social_RepHigh"), 
  prior(normal(0,1.5), class = b, coef = "Social_RepLow"),
  prior(normal(0,1.5), class = b, coef = "BotGenderM"), 
  prior(normal(0,.3), class = sd)
) 
## Testing the prior
#p <- inv_logit(rnorm(10000,0,.8)+ rnorm(10000,0,.8)+ rnorm(10000,0,.8)+ rnorm(10000,0,.8) +rnorm(10000,0,.3))
...
## Standard pp_check
#pp_check(m3.5_prior, nsamples = 100)

## Better pp_check
#y_pred <- posterior_linpred(m3.5_prior) #gives a linear representation, somehow
#dens(inv_logit(y_pred)) #densityplot that has been inverse logit-transformed
...
# - trace plots and trace rank plots
#color_scheme_set("viridis")
#mcmc_trace(m3.5) + theme_classic()
#mcmc_rank_overlay(m3.5) + theme_classic()
## Better pp_check
#y_pred <- posterior_linpred(m3.5)
#dens(inv_logit(y_pred))
## Posterior update check
#plot(hypothesis(m3.5,"Social_RepHigh:BotGenderF > 0"))
```
...
loo_compare(m0, m1, m2, m3, m3.5)
compare(m0, m1, m2, m3, m3.5)

loo_model_weights(m0, m1, m2, m3, m3.5)
...
##### Model 4.5
```{r}
get_prior(virk_4.5, d, family = bernoulli())
prior_f4.5 <- c(
  prior(normal(0,1.5), class = b),
  prior(normal(0,.3), class = sd),
  prior(lkj(1), class=cor)
...
loo_compare(m3, m4, m4.5)
loo_model_weights(m3, m4, m4.5)
...
loo_compare(m3, m4.5, m7, m8, m9, m10, m11, m12)
loo_model_weights(m3, m4.5, m7, m8, m9, m10, m11, m12)

```

##### Model 14
```{r}
get_prior(virk_f14, d, family = bernoulli())
prior_f14 <- c(
  prior(normal(0,1.5), class = b),
  prior(normal(0,.3), class = sd),
  prior(lkj(1), class=cor)
...
## Standard pp_check
pp_check(m14_prior, nsamples = 100)
## Better pp_check
y_pred <- posterior_linpred(m14_prior) #gives a linear representation, somehow
dens(inv_logit(y_pred)) #densityplot that has been inverse logit-transformed
# Fitting the model
m14 <- brm(
  virk_f14,
...
##### MODEL COMPARISON 4 #########
```{r}
m14 <- add_criterion(m14, c("loo", "waic"))



loo_compare(m3, m4.5, "best fra phase 3", m14)
loo_model_weights(m3, m4.5, "best fra phase 3", m14)
...
#######################################################################################################
############################ EXTRA STUFF WE USED TO CHECK STUFF WITH ##################################
